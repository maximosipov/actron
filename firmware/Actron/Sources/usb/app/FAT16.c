...
*  File Name: FAT16.c
*
*  Purpose: This file is for a USB CDC_MSD demo. This file
...
extern const int_8 inf_data[];
extern const uint_32 inf_data_size;
/********************************************************************
*********************************************************************
*       FAT16 Boot Sector
*********************************************************************
...
*********************************************************************
********************************************************************/
const uint_8 FAT16_ConfigFileName[FATFileNameSize]=
{
    'F','S','L','_','V','C','O','M','I','N','F'    /*00-10 - Short File Name */
};
const uint_8 FAT16_RootDirSector[FATDirSize]=
{
    ...
    0x92,           /*24 - Write Date */
    0x38,           /*25 - Write Date */
    0x02,           /*26 - First Cluster (none, because file is empty) */
    0x00,           /*27 - First Cluster (none, because file is empty) */
    0x3A,           /*28 - File Size */
    0x0C,           /*29 - File Size */
    0x00,           /*30 - File Size */
    0x00,           /*31 - File Size */
    'F','S','L','_','M','S','D','D','E','M','O',  /*32-42 - Volume label */
...
)
{
    ...
    case FATRootDirSec0:
        ...
        {
            *pu8DataPointer++ = FAT16_ConfigFileName[i];
        } /* EndFor */
    ...
    case FATDataSec0:
        for(i=0;i<512;i++)
        {
            *pu8DataPointer++ = inf_data[i];
        }
        break;
    case (FATDataSec0+1):
        for(i=0;i<512;i++)
        {
            *pu8DataPointer++ = inf_data[i+512];
        }
        break;
    case (FATDataSec0+2):
        for(i=0;i<512;i++)
        {
            *pu8DataPointer++ = inf_data[i+1024];
        }
        break;
    case (FATDataSec0+3):
        for(i=0;i<512;i++)
        {
            *pu8DataPointer++ = inf_data[i+1536];
        }
        break;
    case (FATDataSec0+4):
        for(i=0;i<512;i++)
        {
            *pu8DataPointer++ = inf_data[i+2048];
        }
        break;
    case (FATDataSec0+5):
        for(i=0;i<512;i++)
        {
            *pu8DataPointer++ = inf_data[i+2560];
        }
        break;
    case (FATDataSec0+6):
        for(i=0;i<inf_data_size - 3072;i++)
        {
            *pu8DataPointer++ = inf_data[i+3072];
        }
        break;
